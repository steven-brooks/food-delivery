...
struct RestaurantService {
	func ownedBy(owner: Owner) -> AnyPublisher<[Restaurant], Never> {
		...
		components.queryItems = [URLQueryItem(name: "orderBy", value: "\"ownerId\""), URLQueryItem(name: "equalTo", value: "\"\(owner.id)\"")]
		...
		return URLSession.shared.dataTaskPublisher(for: request)
			...
			.map {  Array($0.values.sorted(by: { $0.name < $1.name })).map { $0.claim(owner: owner) } }
			.replaceError(with: [])
			.eraseToAnyPublisher()
	}
	...
	func add(restaurant: Restaurant) -> AnyPublisher<Restaurant?, Never> {
		...
		return URLSession.shared.dataTaskPublisher(for: request)
			...
			.map { restaurant.owner != nil ? $0?.claim(owner: restaurant.owner!) : $0}
