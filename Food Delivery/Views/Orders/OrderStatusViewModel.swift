...
class OrderStatusViewModel: ObservableObject {
	...
	@Published var blockSuccess: Bool = false {
		didSet {
			if !blockSuccess {
				...
				if restaurant?.blockedUsers.contains(order.userId) ?? false {
	...
	func toggleUserBlocked() {
		if restaurant != nil {
			if restaurant!.blockedUsers.contains(order.userId) {
				restaurant!.blockedUsers.removeAll(where: {$0 == order.userId})
			} else {
				restaurant!.blockedUsers.append(order.userId)
			}
