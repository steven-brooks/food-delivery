...
struct OrderStatusView: View {
	...
	var blockUserAlert: Alert {
		let title = model.restaurant?.blockedUsers.contains(model.order.userId) ?? false ?
	...
	var nameButton: some View {
		Button(action: {model.blockUserAlert = true}) {
			HStack {
				...
				Circle()
					.foregroundColor(model.restaurant?.blockedUsers.contains(model.order.userId) ?? false ? .red : .appGreen)
...
struct OrderStatusView_Previews: PreviewProvider {
    static var previews: some View {
		...
		let diner = Diner(firstName: "Diner", lastName: "Smith", email: "diner")
